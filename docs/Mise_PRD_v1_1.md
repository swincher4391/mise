...
### 3.3 Extension ↔ PWA Communication
...
3. **Extension-hosted mini UI.** The extension has its own minimal popup UI that shows the extracted recipe with a "Save to Mise" button. This provides immediate feedback even when the PWA isn't open and handles edge cases where the hash fragment would exceed URL length limits (very large recipes). The button triggers mechanism 1 or 2.

This is a 2-hour design decision, not a week-long blocker. All three mechanisms use the same serialized Recipe JSON format.
...
| **No structured data found** (all 3 layers fail) | "No recipe data found on this page. You can paste the recipe text below and we'll try to parse it, or enter it manually." Opens the manual entry form with the source URL pre-filled. |
...
| imageUrl | string \| null | Source URL for the recipe photo. Cached by service worker for offline use. |
| imageBlob | Blob \| null | User-uploaded photo (manual entries only). Only used when no source URL exists. |
| nutrition | Nutrition \| null | Per-serving nutrition if available from source. |
| extractionLayer | enum \| null | `jsonld` \| `microdata` \| `heuristic` \| `manual`. Which extraction layer produced this recipe. Null for imported recipes. |
...
| aggregatedQty | number \| null | Total quantity across all planned recipes. |
| unit | string \| null | Normalized unit after aggregation/conversion. |
| category | string | Grocery aisle category (produce, dairy, meat, pantry, etc.). |
...
| Domain name (.recipes TLD) | ~$50/year | mise.recipes or similar. `.recipes` TLDs run $40–60/year, not $12. |
...
The free tier caps at 25 saved recipes. The natural conversion trigger is saving recipe #26 — the moment the user has clearly adopted Mise as their recipe tool. This is a fair value exchange, not a dark pattern.
...
Target: usable in our kitchen in 2 weeks. Fully featured MVP in 4 weeks. Week 1 is "Brittany can use it tonight" quality — a working prototype, not production polish. The long tail of parser edge cases and UI refinement is ongoing.
...
- Test: extract 20 recipes from popular sites (AllRecipes, Food Network, Bon Appétit, Budget Bytes, Serious Eats). Parser accuracy >70% on ingredient strings (targeting 85%+ by Week 3 as fixtures accumulate).
- **Deliverable:** Brittany can paste a URL and read a recipe while cooking. It works. It's not perfect.
...
### 14.3 Week 3: Cooking Mode + Browser Extension + Paprika Import

- Cooking mode: step-by-step full-screen view with large text, ARIA landmarks, keyboard navigation (spacebar/arrows to advance).
...
- Paprika import: reverse-engineer `.paprikarecipes` format (gzipped SQLite), map fields to Mise Recipe schema, validate with a real Paprika export.
- "Report parsing error" button on ingredients (see Section 8.5).
- Test: cook 5 recipes using cooking mode. Timer works. Scaling works in step text. Import a real Paprika library.
- **Deliverable:** The primary cooking interface. Paprika users can switch. This is where daily use starts.
...
The parser has the most rigorous testing because it's the highest-risk component. 500+ fixture strings (growing continuously), each with an expected structured output. Every parser change runs the full fixture suite. Any regression fails the build. New fixtures are added from real recipe sites and from user-reported parsing errors (Section 8.5).
...
- Cross-browser compatibility beyond Chrome and Firefox. Safari PWA quirks are tested manually.
